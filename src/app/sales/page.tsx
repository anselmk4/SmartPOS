        ...
        {filteredSales.length === 0 && filteredHeldOrders.length === 0 ? (
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-slate-100 p-3 space-y-3">
              {/* 1. HELD ORDERS MOBILE CARDS */}
              {(statusFilter === "ALL" || statusFilter === "PENDING") &&
                filteredHeldOrders.map((order) => (
                  <div
                    key={order.id}
                    className="p-4 rounded-2xl bg-purple-50/50 border border-purple-200/80 shadow-2xs space-y-2.5"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono font-bold text-xs text-purple-800">
                            CMD-{order.id.slice(0, 6).toUpperCase()}
                          </span>
                        </div>
                      ...
                      <span className="text-[10px] font-black uppercase text-purple-800 bg-purple-100 border border-purple-300 px-2 py-0.5 rounded-full">
                        En Attente
                      </span>
                    </div>

                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-1 font-bold text-purple-900 bg-purple-100/80 px-2 py-0.5 rounded-lg border border-purple-200 text-[11px]">
                        <Utensils className="w-3 h-3 text-purple-600 shrink-0" />
                        <span>{order.label}</span>
                      </div>
                      <div className="font-bold text-slate-800 text-[11px]">
                        {order.customerName || "Client Comptoir"}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <Link
                        ...
                        className="flex-1 py-2 px-3 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 font-bold text-xs flex items-center justify-center gap-1 shadow-2xs transition-all"
                      >
                      ...
                      <Link
                        ...
                        className="flex-1 py-2 px-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs flex items-center justify-center gap-1 shadow-xs transition-all touch-press"
                      >
                      ...
                      <button
                        ...
                        className="p-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 font-bold text-xs shadow-2xs"
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <div className="font-bold text-slate-900">
                        {cust ? cust.name : "Comptant (Passager)"}
                      </div>
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between text-xs">
                      <div>
                        <span className="text-[10px] text-slate-400 uppercase font-bold block">Total Net</span>
                        <span className="font-black text-slate-900 text-sm">{formatMoney(sale.totalAmount)}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] text-slate-400 uppercase font-bold block">
                          {sale.debtAmount > 0 ? "Reste Dû" : "Payé"}
                        </span>
                        <span className={`font-bold text-xs ${sale.debtAmount > 0 ? "text-rose-600" : "text-emerald-700"}`}>
                          {sale.debtAmount > 0 ? `-${formatMoney(sale.debtAmount)}` : formatMoney(sale.amountPaid)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        ...
                        className="flex-1 py-2 px-3 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors"
                      >
                      ...
                      {cust?.phone && (
                        <a
                          href={`https://wa.me/${cust.phone.replace(/\D/g, "")}?text=${encodeURIComponent(
                            `Bonjour ${cust.name}, merci pour votre achat de ${formatMoney(sale.totalAmount)} chez ${store?.name || "nous"} (Ticket N° ${sale.receiptNumber}).`
                          )}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="py-2 px-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold text-xs flex items-center justify-center gap-1.5"
                        >
